                                    ...
                                    if (monster.contains("zombie") || monster.contains("skeleton")) {
                                        ...
                                        if (mc.world.getBlockState(spawner.getPos().down()).getBlock() == Blocks.BIRCH_PLANKS) {
                                            if (displaycoords.get())
                                                ...
                                                ChatUtils.sendMsg(Text.of("Detected Activated §cWOODLAND MANSION§r Spawner!"));
                                        } else {
                                            if (displaycoords.get())
                                                ChatUtils.sendMsg(Text.of("Detected Activated §cDUNGEON§r Spawner! Block Position: " + pos));
                                            else ChatUtils.sendMsg(Text.of("Detected Activated §cDUNGEON§r Spawner!"));
                ...
                for (BlockPos pos : spawnerPositions) {
                    ...
                    if (playerPos.isWithinDistance(pos, renderDistance.get() * 16)) {
                        ...
                        if (rangerendering.get())
                            render(new Box(new Vec3d(startX + 17, startY + 17, startZ + 17), new Vec3d(endX - 16, endY - 16, endZ - 16)), rangeSideColor.get(), rangeLineColor.get(), shapeMode.get(), event);
            ...
            synchronized (trialspawnerPositions) {
                for (BlockPos pos : trialspawnerPositions) {
                    ...
                    if (playerPos.isWithinDistance(pos, renderDistance.get() * 16)) {
                        ...
                        int endY = pos.getY();
                        int endZ = pos.getZ();
                        if (trialSpawner.get() && rangerendering.get())
                            render(new Box(new Vec3d(startX + 15, startY + 15, startZ + 15), new Vec3d(endX - 14, endY - 14, endZ - 14)), trangeSideColor.get(), trangeLineColor.get(), shapeMode.get(), event);
                        if (deactivatedSpawnerPositions.contains(pos))
                            render(new Box(new Vec3d(startX + 1, startY + 1, startZ + 1), new Vec3d(endX, endY, endZ)), despawnerSideColor.get(), despawnerLineColor.get(), shapeMode.get(), event);
                        else

...
import static dev.journey.PathSeeker.utils.PathSeekerUtil.logError;
...
public class SignHistorian extends Module {
            ...
            new BoolSetting.Builder()
                    ...
                    .onChanged(it -> {
                        if (it && PathSeekerUtil.checkOrCreateFile(mc, BLACKLIST_FILE)) {
                            ...
                            initBlacklistText();
                            if (mc.player != null) {
                                mc.player.sendMessage(Text.of("§8<" + PathSeekerUtil.randomColorCode() + "§o✨§r§8> §7Please write one blacklisted item for each line of the file."), false);
                                mc.player.sendMessage(Text.of("§8<" + PathSeekerUtil.randomColorCode() + "§o✨§r§8> §7Spaces and other punctuation will be treated literally."), false);
                                mc.player.sendMessage(Text.of("§8<" + PathSeekerUtil.randomColorCode() + "§o✨§r§8> §7You must toggle this setting or the module after updating the blacklist's contents."), false);
                            }
                        }
                    })
                    .build()
    ...
    private void initBlacklistText() {
        ...
        try (Stream<String> lineStream = Files.lines(blackListFile.toPath())) {
            ...
            logError("[PathSeeker] Failed to read from " + blackListFile.getAbsolutePath() + "! - Why:\n" + err);
        }
    }
        ...
        try (Stream<String> lineStream = Files.lines(signsFile)) {
            ...
            for (String sign : entries) {
                try {
                    ...
                    logError("[PathSeeker] Failed to parse SignBlockEntity Nbt: " + err);
                }
            }
        } catch (Exception e) {
            logError(e.toString());
    ...
    public void onActivate() {
        ...
        if (contentBlacklist.get() && PathSeekerUtil.checkOrCreateFile(mc, BLACKLIST_FILE)) initBlacklistText();
    }
        ...
        for (SignBlockEntity sbe : modifiedSigns) {
            if (event.result.getBlockPos().isWithinDistance(sbe.getPos(), 1)) {
                mc.player.sendMessage(
                        Text.of("§8<" + PathSeekerUtil.randomColorCode() + "✨§8> §e§lOriginal§7§l: §7§o" + Arrays.stream(sbe.getFrontText().getMessages(false)).map(Text::getString).collect(Collectors.joining(" "))),
                        false
                );
                mc.player.sendMessage(
                        Text.of("§8<" + PathSeekerUtil.randomColorCode() + "✨§8> §6§lWoodType§7§l: " + ((AbstractSignBlock) sbe.getCachedState().getBlock()).getWoodType().name()
    ...
    private void onBlockAttack(PacketEvent.Send event) {
        ...
        for (SignBlockEntity ghost : destroyedSigns) {
            if (packet.getPos().isWithinDistance(ghost.getPos(), 1.5)) {
                mc.player.sendMessage(
                        Text.of("§8<" + PathSeekerUtil.randomColorCode() + "✨§8> §c§lOriginal§7§l: §7§o" + Arrays.stream(ghost.getFrontText().getMessages(false)).map(Text::getString).collect(Collectors.joining(" "))),
                        false
                );
                mc.player.sendMessage(
                        Text.of("§8<" + PathSeekerUtil.randomColorCode() + "✨§8> §6§lWoodType§7§l: " + ((AbstractSignBlock) ghost.getCachedState().getBlock()).getWoodType().name()
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (timer > 4) {
            ...
            if (!signsToWax.isEmpty()) {
                ...
                List<SignBlockEntity> toModify = modifiedSigns
                        .stream()
                        .filter(sbe -> sbe.getPos().isWithinDistance(mc.player.getBlockPos(), 6))
                        .toList();

                if (!toModify.isEmpty()) {
                    SignBlockEntity sbe = toModify.get(0);
                    interactSign(sbe, Items.BLACK_DYE);
                }
            }
        }
    }
